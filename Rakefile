...
namespace :travis do
  ...
  task :after_success do
    ...
    try "git config --global user.name ${LeNPaul}"
    try "git config --global user.email ${l.nguyen.paul@gmail.com}"
    try "git remote set-url origin \"https://${LeNPaul}@github.com/${LeNPaul}.git\""
